...
REGION_UNIT: t.Dict[str, Unit] = {
    "US": Unit.MILES,
    "UK": Unit.MILES,
}

POINT_NEMO_HEADERS: t.Dict[str, str | float] = {
    "country_code": "N/A",
    "city": "Point Nemo",
    "country_name": "N/A",
    "region_code": "N/A",
...
TESTING_HEADERS: t.Dict[str, str | float] = {
    "city": "Saint-Eugene",
    "country_code": "CA",
    "country_name": "Canada",
    "latitude": 45.8103,
...
class FlaskGeography:
    ...
    _backwards_compatible: bool = None  # type: ignore
    _use_country_code_comparison: bool = None  # type: ignore
    _include_absolute_closest: bool = None  # type: ignore
    _route: t.Optional[str] = None
    _cache: t.Dict = None  # type: ignore

    def __init__(self, app: t.Optional[Flask] = None, *, route: t.Optional[str] = None):
        self._cache = {}
        if app:
    ...
    def use_country_code_comparison(self):
        ...
        self._use_country_code_comparison = str2bool(
            self.app.config.get("GEOGRAPHY_USE_COUNTRY_CODE_COMPARISON", True)
        )
        return self._use_country_code_comparison
    ...
    def include_absolute_closest(self):
        ...
        self._include_absolute_closest = str2bool(
            self.app.config.get("GEOGRAPHY_INCLUDE_ABSOLUTE_CLOSEST", True)
        )
        return self._include_absolute_closest
    ...
    def backwards_compatible(self):
        ...
        self._backwards_compatible = str2bool(
            self.app.config.get("GEOGRAPHY_BACKWARDS_COMPATIBLE", False)
        )
        return self._backwards_compatible
    ...
    def init_app(self, app: Flask, *, route: t.Optional[str] = None):
        self.app = app
        ...
        if not self.route:
            self.app.logger.warning(
                "Cannot instantiate FlaskGeography without a route defined"
            )
            return
        ...
        def init_response():
            ...
            if (
                arg_include_absolute_closest
            ):  # Truthy because of strings, regardless of value
                include_absolute_closest = arg_include_absolute_closest not in [
                    "false",
                    "0",
                    "",
                ]
            ...
            if (
                arg_backwards_compatible
            ):  # Truthy because of strings, regardless of value
                backwards_compatible = arg_backwards_compatible not in [
                    "false",
                    "0",
                    "",
                ]
        ...
        def error_handling_decorator(obj):
        ...
        @app.route(self.route, endpoint="geography")
        ...
        @app.route(
            f"{self.route}/closest-to-user/<path:filename>",
            methods=["GET"],
            endpoint="closest_to_user",
        )
        @error_handling_decorator
        def closest_to_user(filename):
            if not app.extensions["s3_proxy"]:
                return abort(404)
            ...
            if filename not in self._cache or force_refresh:
                try:
                    data_file = app.extensions["s3_proxy"].get_file(f"{filename}.json")
        ...
        @app.route(
            f"{self.route}/closest-to-user", methods=["POST"], endpoint="distances"
        )
        @error_handling_decorator
        def distances():
...
class FlaskGeographyResponse:
    ...
    cf_headers: t.Dict[str, str | float] = None  # type: ignore
    incountry_points = None  # type: ignore
    absolute_closest = None  # type: ignore
    points = None  # type: ignore
    home = None  # type: ignore
    unit: Unit = None  # type: ignore
    country_code_comparison: bool = None  # type: ignore
    include_absolute_closest: bool = None  # type: ignore
    backwards_compatible: bool = None  # type: ignore
        ...
        *,
        ...
        for header, value in request.headers:
            if header.lower() in DESIRED_HEADERS:
                ...
                self.cf_headers.update({DESIRED_HEADERS[header.lower()]: value})
        ...
        if not self.cf_headers:
            ...
            self.cf_headers = (
                TESTING_HEADERS if current_app.debug else POINT_NEMO_HEADERS
            )
        ...
        self.home = {}
        self.unit = (
            REGION_UNIT.get(self.cf_headers.get("country_code", "")) or Unit.KILOMETERS
        )
    ...
    def closest_to_user(self, data):
        ...
        self.home = data.get(
            "home",
            (
                self.cf_headers["latitude"],
                self.cf_headers["longitude"],
            ),
        )
        self.absolute_closest = []
    ...
    def calculate_distance(self, id_, ll, country_code=None):
        ...
        if country_code is not None and country_code == self.cf_headers["country_code"]:
