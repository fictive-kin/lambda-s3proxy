...
class FlaskS3Proxy:
    ...
    _trailing_slash_only: bool = None  # type: ignore
    _trailing_slash_redirection: bool = None  # type: ignore
    _redirect_code: int = None  # type: ignore
    _routes: list = None  # type: ignore
    _subroutes: t.Optional[t.Dict[str, str]] = None
    _locales: t.Optional[t.List] = None
    switchover_proxy: t.Optional[FlaskGradualSwitchoverProxy] = None
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.switchover_proxy = switchover_proxy
        if app:
            self.init_app(app, paths=paths, **kwargs)

    def set_options(
        self, *, bucket: t.Optional[str] = None, prefix: t.Optional[str] = None
    ):
        if bucket is not None:
            self.bucket = bucket
        if prefix is not None:
    ...
    def bucket(self) -> str:
        ...
        if self._bucket is None:
            raise ValueError("FlaskS3Proxy has no bucket configured")
    ...
    @property
    def routes(self) -> t.List[str] | None:
    ...
    def init_app(
        ...
        *,
        ...
        prefix=None,
        paths=None,
        switchover_proxy=None,
        **kwargs,
    ):
        self.app = app
        if switchover_proxy is not None:
            self.switchover_proxy = switchover_proxy
    ...
    def add_handled_routes(self, paths, **kwargs):
        ...
        def path_to_check(path):
            return (
                "/<path:"
                if path.startswith("/<path:") and "/" not in path[1:]
                else path
            )
        ...
        configured_paths = [
            path_to_check(rule.rule) for rule in self.app.url_map.iter_rules()
        ]
    ...
    def _view_func(self):
        if (
            self.switchover_proxy is not None
            and hasattr(self.switchover_proxy, "selector")
            and callable(self.switchover_proxy.selector)
        ):
            return partial(self.switchover_proxy.selector, self.proxy_it)
        return self.proxy_it
    ...
    def retrieve(self, url, *, abort_on_fail=True):
        ...
        try:
            ...
            if content_length == 0 and s3_url in (
                "soap",
                "soap/",
                "soap.html",
                "soap/index.html",
            ...
            if content_length > OVERFLOW_SIZE:
                ...
                url = self.client.generate_presigned_url(
                    "get_object",
                    Params={"Bucket": self.bucket, "Key": s3_url},
                    ExpiresIn=60,
                )
            ...
            if "LastModified" in s3_obj:
                response.headers["Last-Modified"] = self.datetime_to_header(
                    s3_obj["LastModified"]
                )
            return response
            ...
            self.app.logger.info(
                "Unable to open: {}/{}: {}".format(self.bucket, s3_url, exc)
            )
            pass
    ...
    def setup_subroutes(self, *, file=None, subroutes=None):
        ...
        if self.subroutes:
            for route, bucket in self.subroutes.items():
                ...
                self.app.logger.info(
                    f"Instantiating sub-route: {route} -> {bucket}{prefix[0]}"
                )
    ...
    def setup_locales(self, *, file=None, locales=None, enable_auto_switch=None):
        ...
        if self.locales:
            ...
            for locale in set(self.locales):
                self.app.logger.info(
                    f"Instantiating locale-specific blueprint for {locale}"
                )
