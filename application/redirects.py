...
class FlaskJSONRedirects:
    ...
    def init_app(self, app: Flask, *, file: str = None):
        """
        ...
        if not self.spec_dict and file is None:
            app.logger.warning(
                'REDIRECTS_SPEC_DICT is not set and no filename was provided. No redirects will'
                ' be setup automatically.')
            return

        if self.spec_dict:
            self.process_redirects(self.spec_dict)
        if file is not None:
    ...
    def create_redirect(self, uri, target, *,
        ...
        status_code = status_code if status_code is not None else self.default_status_code
